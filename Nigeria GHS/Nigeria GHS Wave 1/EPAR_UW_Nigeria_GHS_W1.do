...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------
*Title/Purpose 	: Agricultural Development Indicators for the LSMS-ISA, Nigeria General Household Survey (GHS) LSMS-ISA Wave 1 (2010-11)
...
*FINAL FILES CREATED
*-------------------------------------------------------------------------------------
*HOUSEHOLD VARIABLES				Nigeria_GHS_W1_household_variables.dtac
...
gen dummy = _n
reshape long qty value unit, i(hhid crop_code dummy) j(idno)
drop idno dummy
ren qty quantity_sold
ren val sales_value
*merging in conversion factors
merge m:1 crop_code unit using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_ng3_cf.dta", gen(cf_merge)
...
*/ 
//Eventual solution pending something more reproducible.
...
preserve
keep zone state lga sector ea hhid plot_id indiv* weeks_worked* days_week*
reshape long indiv weeks_worked days_week, i(zone state lga sector ea hhid plot_id) j(colid) string
drop colid 
drop if indiv==. | weeks_worked==. | days_week==0
replace weeks_worked=52 if weeks_worked > 52
replace days_week=7 if days_week > 7
gen season="ph"
...
restore
	...
	gen season="ph"
	keep zone state lga sector ea hhid plot_id season daysnonhired*
...
use `harvest_family', clear
gen days=weeks_worked*days_week
drop if days==. | days==0
merge m:1 hhid indiv using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_person_ids.dta", nogen keep(1 3)
...
save "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_hh_cost_labor.dta", replace
	...
	*Fertilizer
	...
	recode qty (.=0)
	replace input="inorg" if itemcode!=3 & !strmatch(input, "transfert")
	replace input="orgfert" if itemcode==3
	collapse (sum) qty* val*, by(zone state ea lga sector hhid plot_id exp input itemcode)
	tempfile phys_inputs
	save `phys_inputs'
	
	//We can estimate how many nutrient units were applied for most fertilizers; dry urea is 46% N and NPK can have several combinations. We go with a weighted average of 18-12-11 based on https://africafertilizer.org/#/en/vizualizations-by-topic/consumption-data/
	...
	gen n_org_kg = qty*(itemcode==3)*0.01
	...
	gen inorg_fert_kg = qty*(itemcode==1)+qty*(itemcode==2)+qty*(itemcode==4)
	gen org_fert_kg = qty*(itemcode==3)
	...
	*/ 				(3113 3112 3111 = 3110) (3022=3020) (2142 2141 = 2140) (1121 1122 1123=1120)
	...
	replace unit = 31 if unit==32 //Both large mudu, not sure where the surplus unit code came from.
	...
	replace conversion = 10 if crop_code==1020 & inrange(unit,160,162) //10 stems per bundle, regardless of size
	replace conversion = 1 if inrange(unit,80,82) | (unit==1 & conversion==.) //pieces //why there are kgs not converted at this point is beyond my ken. 
	replace unit=1 if inlist(unit,160,161,162,80,81,82) //pieces
...
save "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_hh_cost_inputs_long.dta", replace //ALT 02.07.2022: Holdover from W4.
restore
preserve
...
drop val_mech_imp* val_seedtrans_imp* val_feedanml_imp* //Not going to have any data
...
merge m:1 crop_code unit using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_ng3_cf.dta", nogen keep(1 3) //Need to implement conversions here.
merge m:1 hhid crop_code unit using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_hh_crop_prices_for_wages.dta", keep(1 3) nogen
//merge m:1 hhid crop_code using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_hh_crop_prices_kg.dta", keep(1 3) nogen //to create
merge m:1 crop_code unit using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_crop_prices_median_country.dta"
gen price_received = value_processed_crop_sold / quant_processed_crop_sold
gen price_as_input = hh_price_mean
replace price_as_input = price_unit_country if price_as_input==.d
...
use "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_all_plots.dta", clear
collapse (sum) ha_planted, by(hhid plot_id)
gen cultivated=1 //Anything that ends up in all plots has been cultivated, although ha_planted is sometimes unknown
merge 1:1 hhid plot_id using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_plot_areas.dta", nogen keepusing(field_size rented_out)
replace cultivated=0 if cultivated==. | rented_out==1
...
gen farm_area=ha_planted 
replace farm_area=field_size if ha_planted==. & cultivated==1
gen land_size=field_size if cultivated==1 | rented_out==0
replace land_size=farm_area if land_size==.
gen farm_size_agland=field_size
replace farm_size_agland=farm_area if farm_size_agland==.
collapse (sum) farm_area land_size farm_size_agland, by (hhid)
la var farm_area "Actively cultivated area (ha)"
la var land_size "All area held by household (ha)"
la var farm_size_agland "All area owned by household, including area rented out (ha)" //Will be almost the same as land_size
save "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_land_size.dta", replace

*******************************************************************************
*LAND SIZE
********************************************************************************
//AT: already captured in the above section.
...
gen indiv=farmerid
drop if indiv==.
merge 1:1 hhid indiv using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_person_ids.dta", nogen
...
recode use* (.=0)
/* not used because there's nothing here.
preserve 
keep hhid plot_id crop_code 
gen imprv_seed_ = .
gen hybrid_seed_ = .
...
merge m:m hhid plot_id using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_dm_ids.dta", nogen keep(1 3) //Many-to-many merges are unpopular because the odds of accidentally inflating values by collapsing on duplicates goes way up; however, we only need to know whether *any* person managing a given plot used <input> for *at least one* crop grown on that plot. In this case the duplication is not improper.
collapse (max) use_*, by(hhid indiv female) //Going to be 100% empty for improved seed
	la var use_inorg_fert "1= Farmer uses inorganic fertilizer"
	la var use_pest "1 = Farmer uses pesticide"
	la var use_herb "1 = Farmer uses herbicide"
	la var use_org_fert "Farmer uses organic fertilizer (NA for W1)"
	gen farm_manager=1 if indiv!=.
	recode farm_manager (.=0)
	lab var farm_manager "1=Indvidual is listed as a manager for at least one plot" 
	save "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_farmer_input_use.dta", replace
preserve
	...
	la var use_org_fert "Household uses organic fertilizer (NA for W1)"
	gen use_imprv_seed=.
	la var use_imprv_seed "1=household uses improved or hybrid seeds (NA for W1)"
	gen use_hybrid_seed = .
	la var use_hybrid_seed "1=household uses hybrid seeds (NA for W1)"
...
* SEED SOURCES *
*******************************************************************************
use "${Nigeria_GHS_W1_raw_data}\sect12_plantingw1.dta", clear
...
// 	drop if purch_ == 0
// 	drop purch_
	save "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_purchasedsources_long.dta", replace
restore
...
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_weights.dta", nogen keep (2 3)
save "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_purchasedsources.dta", replace
...
preserve
	...
	merge 1:1 hhid crop_code using "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_purchasedsources_long.dta", nogen
	...
	* reconcile shares if households only report using seeds from a single source but dont report actual quantity of seed used
	...
	save "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_seed_sources_long.dta", replace
restore
...
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_weights.dta", nogen keep (2 3)
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_purchasedsources.dta", nogen
...
save "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_seed_sources.dta", replace
...
recode *kg (.=0)
//ren *_rate *_kg_
gen dm_gender2="unknown" //10 plots still to be resolved.
...
preserve
	...
	replace all_area_harvested=all_area_planted if all_area_harvested>all_area_planted & all_area_harvested!=. //no changes
...
//merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_trees.dta", nogen
...
foreach p of global topcropname_area {
	lab var grew_`p' "1=Household cultivated `p'"
...
replace crop_group=	"Yam"	if crop_code==	1124 //Still counting this for commodity classification even if it's not lumped with the other yams
...
*Farm size
//merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_land_size_all.dta", nogen keep (1 3) //consolidated into land_size
//merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_farmsize_all_agland.dta", nogen keep (1 3) //ditto
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_land_size.dta", nogen keep (1 3)
//merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_land_size_total.dta", nogen //land_size_total is equivalent to farmsize_agland and isn't used
...
*Household size
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_weights.dta", nogen keep (1 3)
...
merge 1:1 hhid using "${Nigeria_GHS_W1_created_data}\Nigeria_GHS_W1_seed_sources.dta", nogen keep(1 3)
...
/*
foreach c of global topcropname_area {
	replace yield_pl_`c'=. if w_area_plan_`c'<0.05
	replace yield_hv_`c'=. if w_area_plan_`c'<0.05
	foreach g of global allyield  {
		replace yield_pl_`g'_`c'=. if w_area_plan_`c'<0.05
		replace yield_hv_`g'_`c'=. if w_area_plan_`c'<0.05	
	}
}
*/
...
gen poverty_under_215 = daily_percap_cons < $Nigeria_GHS_W1_poverty_215
la var poverty_under_215 "Household per-capita consumption is below $2.15 in 2017 $ PPP"
gen poverty_under_npl = daily_percap_cons < $Nigeria_GHS_W1_poverty_npl
gen poverty_under_300 = daily_percap_cons < $Nigeria_GHS_W1_poverty_300
...
//merge 1:1 hhid indiv using "${Nigeria_GHS_W1_created_data}/Nigeria_GHS_W1_farmer_input_use.dta", nogen  keep(1 3) //To do
...
gen female_imprv_seed_use=.
gen male_imprv_seed_use=.
lab var male_imprv_seed_use "1 = Individual male farmer (plot manager) uses improved seeds (NA for W1)" 
lab var female_imprv_seed_use "1 = Individual female farmer (plot manager) uses improved seeds (NA for W1)"
