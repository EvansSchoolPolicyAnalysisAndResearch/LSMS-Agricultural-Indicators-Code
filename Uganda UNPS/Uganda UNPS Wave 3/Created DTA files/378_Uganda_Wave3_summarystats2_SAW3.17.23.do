/* Uganda Wave 3 Summary Statistics Final Do-file done by: Sebastian Wood Date: 3/17/2023
Reference: Nigeria Wave 3
*/
clear
clear matrix 
clear mata	 
...
foreach instrument of global list_instruments {
	* List of final files to use in the reporting and output file of summary statistics
	global final_data_household  "${UGS_W3_final_data}/`instrument'_household_variables.dta"  
	global final_data_individual "${UGS_W3_final_data}/`instrument'_individual_variables.dta"
	global final_data_plot       "${UGS_W3_final_data}/`instrument'_field_plot_variables.dta"
	*global final_outputfile      "$`instrument'_final_data/`instrument'_summary_stats_`c(current_date)'.xlsx" 
	global final_outputfile      "${UGS_W3_final_data}/`instrument'_summary_stats.xlsx" 
	...
	*/ $topcrop_monetary 
	...
	foreach p of global monetary_vars {
		*Check if the variable exists and if not generate it with values equal to 0  (in the spreadsheet we will remove all rows for whichl all stats are zero
		...
		qui gen `p'_1ppp = (1+${NPS_LSMS_ISA_W3_inflation}) * `p' / ${NPS_LSMS_ISA_W3_cons_ppp_dollar} 
		qui gen `p'_2ppp = (1+${NPS_LSMS_ISA_W3_inflation}) * `p' / ${NPS_LSMS_ISA_W3_gdp_ppp_dollar}
		qui gen `p'_usd = (1+${NPS_LSMS_ISA_W3_inflation})  * `p' / ${NPS_LSMS_ISA_W3_exchange_rate} 
		qui gen `p'_loc = (1+${NPS_LSMS_ISA_W3_inflation})  * `p' 
		qui local l`p' : var lab `p' 
		qui lab var `p'_1ppp "`l`p'' (2012 $ Private Consumption PPP)"
		qui lab var `p'_2ppp "`l`p'' (2012 $ GDP PPP)"
		qui lab var `p'_usd "`l`p'' (2012 $ USD)"
		qui lab var `p'_loc "`l`p'' (2012 LCU)"
		qui lab var `p' "`l`p'' (LCU)"  
		qui gen w_`p'_1ppp = (1+${NPS_LSMS_ISA_W3_inflation}) * w_`p' / ${NPS_LSMS_ISA_W3_cons_ppp_dollar} 
		qui gen w_`p'_2ppp = (1+${NPS_LSMS_ISA_W3_inflation}) * w_`p' / ${NPS_LSMS_ISA_W3_gdp_ppp_dollar}
		qui gen w_`p'_usd = (1+${NPS_LSMS_ISA_W3_inflation})  * w_`p' / ${NPS_LSMS_ISA_W3_exchange_rate} 
		qui gen w_`p'_loc = (1+${NPS_LSMS_ISA_W3_inflation})  * w_`p' 
		local lw_`p' : var lab w_`p'
		qui lab var w_`p'_1ppp "`lw_`p'' (2012 $ Private Consumption PPP)"
		qui lab var w_`p'_2ppp "`lw_`p'' (2012 $ GDP PPP)"
		qui lab var w_`p'_usd "`lw_`p'' (2012 $ USD)"
		qui lab var w_`p'_loc "`lw_`p'' (2012 LCU)"
...
di "---------------- Summary statistics group 2 ----------------"
	...
	** Group 4  - yields 
	...
	//no Yield harvest indicators for Uganda wave 3, we could exclude them from generating all these variables (all zeros right now)
	...
	*recode weights that could not be created to 1
	...
	if "`instrument'"=="Uganda_NPS_LSMS_ISA_W3" {
		drop livestock_hh
...
*tz tbs and india rms generate bottom_40_peraeq=1 in the instrument do files but we should go back and make that missing so that the .dta files (if we ever were to share those). will go back and fix
	...
	*POVERTY ESTIMATE ALL
	...
	foreach v in poverty_under_1_9_all {
...
foreach instrument of global list_instruments {
	...
	*** Adding other meta-data
	...
	merge 1:1 variablenameinthedtafile using "R:\Project\EPAR\Working Files\335 - Ag Team Data Support\Waves\_Summary_statistics\EPAR_UW_335_master_list_indicators.dta", nogen
	...
	if "`instrument'"=="Uganda_NPS_LSMS_ISA_W3" { 
		...
		gen Survey="LSMS-ISA"
		gen Instrument="Uganda NPS Wave 3"	
		gen Year="2011-12"
	}
	...
	if "`instrument'"=="Nigeria_GHS_W4" { 
		gen Geography = "Nigeria"
		gen Survey="Nigeria GHS W4"
		gen Instrument = "LSMS-ISA"
		gen Year = "2018-19"
	}
	if "`instrument'"=="Tanzania_NPS_W5" {
		gen Geography="Tanzania"
		gen Survey = "Tanzania NPS W5"
		gen Instrument = "LSMS-ISA"
		gen Year = "2019-20"
	}
	...
	*drop row_num
	...
	save "${UGS_W3_final_data}/`instrument'_summary_stats_with_labels.dta", replace
}
...
*************************************STOP ***********************************************
*Summary Statistics group 5, 11, 12, not working. 