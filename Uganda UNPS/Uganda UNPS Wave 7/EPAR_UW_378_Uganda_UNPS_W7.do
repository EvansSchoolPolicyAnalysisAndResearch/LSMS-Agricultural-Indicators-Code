...
*Date			: This  Version - April 4, 2024
----------------------------------------------------------------------------------------------------------------------------------------------------*/
...
global root_folder "R:/Project/EPAR/Working Files/378 - LSMS Burkina Faso, Malawi, Uganda/uganda-wave7-2018-19"
global directory "R:\Project\EPAR\Working Files\378 - LSMS Burkina Faso, Malawi, Uganda\_SUMMARY_STATS"
global Uganda_NPS_W7_raw_data 			"${root_folder}/Raw DTA File"
global Uganda_NPS_W7_created_data 		"${root_folder}/Final DTA Files\created_data"
global Uganda_NPS_W7_final_data  		"${root_folder}/Final DTA Files\final_data"
...
/* ALT MERGE RESOLUTION: Do we need this?
********************************************************************************
...
********************************************************************************
use "${Uganda_NPS_W7_raw_data}/HH/GSEC2.dta", clear
ren PID personid			
gen female =h2q3==2
ren h2q8 age
...
tostring HHID, format(%18.0f) replace
merge m:1 HHID personid using "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_gender_merge.dta", gen(dm1_merge) keep(1 3) //9442 missing personid...
...
// Loop for price per kg for each crop at different geographic dissagregate using Standard price per kg variable (from standar conversion factor table)
	foreach i in region district county /*subcounty*/ parish /*ea*/ HHID {
		...
		collapse (median) price_unit_`i' = price_unit [aw=weight], by  (crop_code /*sold_unit_code*/ `i' obs_`i'_price)
...
merge 1:1 hhid crop_code using "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_cropsales_value.dta"
...
* No individual data in the raw data even though in the questionnaires
*They didn't include this stuff so this data is redundant. 
use "${Uganda_NPS_W7_raw_data}/Agric/AGSEC3A.dta", clear
gen season = 1
append using "${Uganda_NPS_W7_raw_data}/Agric/AGSEC3B.dta"
replace season = 2 if season == .
...
ren pltid plot_id
ren parcelID parcel_id
rename s3bq03_3 indiv //indiv
destring indiv, replace 
*rename indiv_*_* days_*
keep hhid parcel_id plot_id season indiv /*days_* days fam_worker_count*/
...
duplicates drop hhid parcel_id plot_id season, force //  3 observations deleted 
*reshape long indiv days, i(hhid parcel_id plot_id season) j(colid) string no need to reshape since its already in that format 
*drop if days == .
...
keep region district county /*subcounty ea*/ parish  hhid parcel_id plot_id season gender  labor_type /*fam_worker_count days*/
...
foreach i in region /*sregion*/ district county parish hhid /*ea*/{
preserve
...
save "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_hh_monocrop_plots.dta", replace 
...
replace livestockid = ALiveStock_Small_ID if livestockid==.
replace livestockid = APCode if livestockid==.
merge m:1 livestockid using "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_tlu_all_animals.dta", nogen
label val livestockid livestockid //have to reassign labels to values after creating new variable
label var livestockid "Livestock Species ID Number"
...
ren a3aq36 wagehired // no wages reported in season 2 data 
*replace wagehired = a3bq36 if wagehired == .
gen wagehiredmale = wagehired if dayshiredmale != .
gen wagehiredfemale = wagehired if dayshiredfemale != .
gen wagehiredchild = wagehired if dayshiredchild != .
...
keep hhid region rural weight district county county_name parish parish_name /*ea*/ price_per_animal number_sold income_live_sales number_slaughtered value_livestock_purchases livestock_code
...
use "${Uganda_NPS_W7_raw_data}/HH/gsec12_2.dta", clear
rename hhid HHID
...
use "${Uganda_NPS_W7_raw_data}/HH/GSEC8.dta", clear
...
*generating field_size
gen parcel_area = s2aq4  
replace parcel_area = s2aq04 if parcel_area == . //only one way data available 
replace parcel_area = s2aq5 if parcel_area == . // 966 values input from farmer estimates (introduces noise in later analysis)
replace parcel_area = s2aq05 if parcel_area == . // 538 changes
gen field_size = plot_area_pct*parcel_area //using calculated percentages of plots (out of total plots per parcel) to estimate plot size using more accurate parcel measurements
replace field_size = field_size/2.471 //convert acres to hectares
...
**
save "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_parcels_cultivated.dta", replace
merge 1:1 HHID parcel_id using "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_parcel_areas.dta", nogen keep (1 3) // only 700 matches 
...
merge 1:m hhid parcel_id plot_id using "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_all_plots.dta", nogen keep(1 3) keepusing(crop_code)
...
save "${Uganda_NPS_W7_created_data}/Uganda_NPS_W7_imprvseed_crop.dta", replace //ALT: this is slowly devolving into a kludgy mess as I try to keep continuity up in the hh_vars section.
restore
...
use "${Uganda_NPS_W7_raw_data}/HH/GSEC14.dta", clear
...
*LAND RENTAL 
********************************************************************************
//ALT merge resolution: We should be able to do it this way versus the much longer formatting in the old code. Follow up to make sure it works.
...
use "${Uganda_NPS_W7_raw_data}/HH/GSEC15B.dta", clear
