...
foreach instrument of global list_instruments {
	* List of final files to use in the reporting and output file of summary statistics
	global final_data_household  "${Uganda_NPS_W4_final_data}/`instrument'_household_variables.dta"  
	global final_data_individual "${Uganda_NPS_W4_final_data}/`instrument'_individual_variables.dta"
	global final_data_plot       "${Uganda_NPS_W4_final_data}/`instrument'_field_plot_variables.dta"
	*global final_outputfile      "$`instrument'_final_data/`instrument'_summary_stats_`c(current_date)'.xlsx" 
	global final_outputfile      "${Uganda_NPS_W4_final_data}/`instrument'_summary_stats.xlsx" 
	...
	*/ $topcrop_monetary 
	...
	foreach p of global monetary_vars {
		*Check if the variable exists and if not generate it with values equal to 0  (in the spreadsheet we will remove all rows for whichl all stats are zero
		...
		qui gen `p'_1ppp = (1+${UGA_W4_inflation}) * `p' / ${UGA_W4_cons_ppp_dollar} 
		qui gen `p'_2ppp = (1+${UGA_W4_inflation}) * `p' / ${UGA_W4_gdp_ppp_dollar}
		qui gen `p'_usd = (1+${UGA_W4_inflation})  * `p' / ${UGA_W4_exchange_rate} 
		qui gen `p'_loc = (1+${UGA_W4_inflation})  * `p' 
		...
		qui gen w_`p'_1ppp = (1+${UGA_W4_inflation}) * w_`p' / ${UGA_W4_cons_ppp_dollar} 
		qui gen w_`p'_2ppp = (1+${UGA_W4_inflation}) * w_`p' / ${UGA_W4_gdp_ppp_dollar}
		qui gen w_`p'_usd = (1+${UGA_W4_inflation})  * w_`p' / ${UGA_W4_exchange_rate} 
		qui gen w_`p'_loc = (1+${UGA_W4_inflation})  * w_`p' 
		local lw_`p' : var lab w_`p'
		qui lab var w_`p'_1ppp "`lw_`p'' (2017 $ Private Consumption PPP)"
		qui lab var w_`p'_2ppp "`lw_`p'' (20127 $ GDP PPP)"
...
foreach instrument of global list_instruments {
	...
	*drop row_num
	...
	save "${Uganda_NPS_W4_final_data}/`instrument'_summary_stats_with_labels.dta", replace
}
