...
/*-----------------------------------------------------------------------------------------------------------------------------------------------------
*Title/Purpose 	: Agricultural Development Indicators for the LSMS-ISA,Tanzania National Panel Survey (TNPS) LSMS-ISA Wave 3 (2012-13)
...
drop rnum
save "${Tanzania_NPS_W3_created_data}/Tanzania_NPS_W3_cropname_table.dta", replace 
...
bys y3_hhid plot_id season : egen total_prop_planted=sum(prop_planted)
...
ren ag3a_62_1 qtypestherbexp0 //0-1 designators for long/season growing seasons.// quantity of the pesticide used
...
foreach cn in $topcropname_area {
    ...
    // drop exp
    // capture confirm file "${Tanzania_NPS_W3_created_data}/Tanzania_NPS_W3_`cn'_monocrop.dta"
    // if !_rc {
    ...
    if (r(N) > 0) {  // Ensure collapse only happens if observations exist
        ...
        // To do: labels
        save "${Tanzania_NPS_W3_created_data}/Tanzania_NPS_W3_inputs_`cn'.dta", replace
    }
...
//Fix: This could get integrated into all plots. 
...
//leftover/free inputs. For TZA W5, only organic fertilizer and seed have an implicit component, so this is overkill (labor is probably the biggest implicit cost and is missing from this wave). However,
//I leave it in because it allows median prices to be easily extracted if for some reason that information becomes relevant, and it makes the code more portable.
ren ag3b_62_1 qtyherbpestexp //The label says long season, but I'm pretty sure these are season  inputs?
ren ag3b_62_2 unitherbpestexp
...
*/
foreach i in herbpestexp  { //
	foreach j in 0 1 {
	replace qty`i'=qty`i'/1000 if unit`i'==3 & qty`i'>9 //Assuming instances of very small amounts are typos. 
	replace unit`i'=2 if unit`i'==3
		}
	}
ren ag3b_44 qtyorgfertexp
...
*/
//Price disaggregation of inorganic fertilizer isn't necessary, because there's no implicit inorganic fertilizer useage
ren ag3b_49 qtyinorgfertexp1 
ren ag3b_56 qtyinorgfertexp2
ren ag3b_48 input1
ren ag3b_55 input2
ren ag3b_51 valinorgfertexp1
ren ag3b_58 valinorgfertexp2

egen qtyinorgfertexp=rowtotal(qtyinorgfertexp2 qtyinorgfertexp1)
egen valinorgfertexp=rowtotal(valinorgfertexp1 valinorgfertexp2) 
...
label values input "input"
decode input, gen (input_inorg) 
gen qty = qtyinorgfertexp

gen qty_org=qtyorgfertexp
*replace qty_org=qtyorgfertexp1 if qty_org==.
gen n_kg = qty*strmatch(input_inorg, "npk_fert")*0.17 + qty*strmatch(input_inorg, "urea")*0.46 + qty*strmatch(input_inorg, "mrp")*0.24
gen p_kg = qty*strmatch(input_inorg, "npk_fert")*0.17 + qty*strmatch(input_inorg, "dap")*0.18 + qty*strmatch(input_inorg, "mrp")*0.32 + qty*strmatch(input_inorg, "TSP")*0.45
gen k_kg = qty*strmatch(input_inorg, "npk_fert")*0.17
...
lab var herbpest_kg "Pesticide (kgs) for household"
...
lab var herbpest_kg_`i' "Pesticide (kgs) for `i'-managed plots"
...
preserve 
use "${Tanzania_NPS_W3_raw_data}/hh_sec_j3.dta" , clear
ren y3_hhid hhid
drop occ
ren hh_j09_3 days 
...
global wins_var_top1_gender $wins_var_top1_gender cost_total cost_expli inorg_fert_kg org_fert_kg n_kg p_kg k_kg dap_kg ha_irr can_kg sa_kg mrp_kg n_org_kg npk_kg urea_kg herbpest_kg wage_paid_aglabor
...
global empty_vars $empty_vars *wage_paid_aglabor*
...
foreach v in inorg_fert org_fert n p k  herbpest urea npk  {
...
lab var herbpest_rate "Rate of pesticide application (kgs/ha) (household)"
...
lab var herbpest_rate_`g' "Rate of pesticide application (kgs/ha) (`g'-managed plots)"
...
gen poverty_under_190 = daily_percap_cons < $Tanzania_NPS_W3_poverty_190
...
*/ ccf_loc ccf_usd ccf_1ppp ccf_2ppp *sales_livestock_products *value_pro* *value_sal* nb*
